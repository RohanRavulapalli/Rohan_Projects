...
class matrix():
    ...
    def transpose(self):
        transpose = []
        while len(transpose) != len(self.matrix[0]):
            transpose.append([])
        for row in self.matrix:
...
def create():
    ...
    cols = input('\nplease specify the number of columns of your matrix: '.upper())
    
    while rows.strip().isnumeric() == False:
        print('\nYou did not provide a valid numeric input for the # of rows')
        rows = input('\nplease specify the number of rows of your matrix: '.upper())
        
    while cols.strip().isnumeric() == False:
        print('\nYou did not provide a valid numeric input for the # of columns')
        cols = input('\nplease specify the number of columns of your matrix: '.upper())
    ...
    for i in range(len(matrix)):
        ...
        while len(row) != cols:
            print('\nThe number of entries you entered does not match the number of columns specified'.upper())
            user = input(f'\nPlease enter the entries of row {i+1} of your matrix (with spaces): '.upper())
            row = user.split(' ') 
...
def operations():
    ...
    while validity == 'b':
        ...
        matrix1 = create()
        print('\nBased on your inputs, is this your first matrix?')
